...
export let execute: EntryPoints.Scheduled.execute = () => {
  ...
  const safetyStockLevelSearch = runtime
    .getCurrentScript()
    .getParameter({ name: 'custscript_sp_safety_stock_tw_ss' }) as string;
  // create search
  ...
  pagedData.pageRanges.forEach(function (pageRange) {
    ...
    page.data.forEach(function (result) {
      ...
      const savedId = record.submitFields({
        ...
        id: id,
        values: {
          custitem_sp_safety_stock_level_tw_date: new Date(),
        },
        options: {
...
const createCSV = (items: Item[]) => {
  const dir = Number(
    runtime
      .getCurrentScript()
      .getParameter({ name: 'custscript_sp_safety_stock_tw_dir' })
  );
  const today = todaysDate();
  const rnd = generateRandomString();
  // create the csv file
  const csvFile = file.create({
    name: 'townsend-safety-stock-level-' + today + '_' + rnd + '.csv',
...
const sendEmail = (items: Item[], fileId: number) => {
  ...
  const emailRecipient = runtime.getCurrentScript().getParameter({
    name: 'custscript_sp_safety_stock_tw_rec',
  }) as string;
  const emailList = String(
    runtime
      .getCurrentScript()
      .getParameter({ name: 'custscript_sp_safety_stock_tw_list' })
  ).split(',');

  let html = `
    <p>The following item's availability is below the safety stock level for Townsend.</p>
    <table>
      <tr>
  ...
  html += `</table><p>You can find the results <a href="https://system.netsuite.com/app/common/search/searchresults.nl?searchid=1424&whence=" target="_blank">here.</a></p>`;
