...
export const _get = (start: string, end: string) => {
  // load search
  const transactionSearch = search.create({
    ...
    columns: [
      ...
      search.createColumn({
        ...
        formula:
          "MAX(CASE WHEN {type} = 'Cash Sale' OR {type} = 'Invoice' THEN NVL({shippingamount},0) END)",
        summary: search.Summary.SUM,
      }),
      search.createColumn({
    ...
    filters: [
      ...
      search.createFilter({
        ...
        values: [1],
        formula:
          "CASE WHEN {type} = 'Cash Sale' OR {type} = 'Invoice' THEN 1 ELSE 0 END",
      }),
      search.createFilter({
        name: 'mainline',
