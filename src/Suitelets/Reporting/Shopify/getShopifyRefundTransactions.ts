...
export const _get = (start: FieldValue, end: FieldValue) => {
  // load search
  ...
  pagedData.pageRanges.forEach(pageRange => {
    ...
    page.data.forEach(result => {
      const refundsCount = Number(
        result.getValue({ name: 'number', summary: search.Summary.COUNT })
      );
      const refundsAmount = Number(
        result.getValue({ name: 'amount', summary: search.Summary.SUM })
      );
      const refundsDiscountAmount = Number(
        result.getValue({
          name: 'discountamount',
          summary: search.Summary.SUM,
        })
      );
      const refundsShippingAmount = Number(
        result.getValue({
          name: 'formulacurrency1',
          summary: search.Summary.SUM,
        })
      );
      const refundsTaxAmount = Number(
        result.getValue({
          name: 'formulacurrency2',
          summary: search.Summary.SUM,
        })
      );
      const refundsAmountNoTax = Number(refundsAmount + refundsShippingAmount);
