...
interface Row {
  salesRep: string;
  lastTotalLeads: number;
  lastConvertedLeads: number;
  lastConversionRate: number;
  lastAvgDaysToClose: number;
  currentTotalLeads: number;
  currentConvertedLeads: number;
  currentConversionRate: number;
  currentAvgDaysToClose: number;
}
...
export const _create = (
  ...
  const data: Data = getCustomerSearchResults(start, end, prevStart, prevEnd);
...
const getCustomerSearchResults = (
  ...
  const lastLeadsConverted = search.createColumn({
    ...
    formula:
      ...
      "', 'MM/DD/YYYY') AND to_date('" +
      prevEnd +
      "', 'MM/DD/YYYY') AND {status} = 'CUSTOMER-Closed Won' THEN 1 ELSE 0 END),0)",
    summary: search.Summary.MAX,
  });
  ...
  const lastLeadsAvgDaysToClose = search.createColumn({
    ...
    formula:
      "NVL(ROUND(SUM(CASE WHEN {datecreated} BETWEEN to_date('" +
      prevStart +
      "', 'MM/DD/YYYY') AND to_date('" +
      prevEnd +
      "', 'MM/DD/YYYY') THEN {dateclosed} - {leaddate} END),2),0)",
  ...
  const lastLeadsConversionRate = search.createColumn({
    ...
    formula:
      "CASE WHEN SUM(CASE WHEN {datecreated} BETWEEN to_date('" +
      prevStart +
      "', 'MM/DD/YYYY') AND to_date('" +
      prevEnd +
      "', 'MM/DD/YYYY') THEN 1 ELSE 0 END) = 0 THEN 0 ELSE SUM(CASE WHEN {datecreated} BETWEEN to_date('" +
      prevStart +
      "', 'MM/DD/YYYY') AND to_date('" +
      prevEnd +
      "', 'MM/DD/YYYY') " +
      "AND {status} = 'CUSTOMER-Closed Won' THEN 1 ELSE 0 END)/SUM(CASE WHEN {datecreated} BETWEEN to_date('" +
      prevStart +
      "', 'MM/DD/YYYY') AND to_date('" +
      prevEnd +
      "', 'MM/DD/YYYY') THEN 1 ELSE 0 END) END",
    summary: search.Summary.MAX,
  });
  ...
  pagedData.pageRanges.forEach(pageRange => {
    ...
    page.data.forEach(result => {
      ...
      const lastTotalLeads = Number(
        result.getValue({
          name: 'formulanumeric1',
          summary: search.Summary.MAX,
        })
      );
      const currentTotalLeads = Number(
        result.getValue({
          name: 'formulanumeric2',
          summary: search.Summary.MAX,
        })
      );
      const lastConvertedLeads = Number(
        result.getValue({
          name: 'formulanumeric3',
          summary: search.Summary.MAX,
        })
      );
      const currentConvertedLeads = Number(
        result.getValue({
          name: 'formulanumeric4',
          summary: search.Summary.MAX,
        })
      );
      const lastAvgDaysToClose = Number(
        result.getValue({
          name: 'formulanumeric5',
          summary: search.Summary.AVG,
        })
      );
      const currentAvgDaysToClose = Number(
        result.getValue({
          name: 'formulanumeric6',
          summary: search.Summary.AVG,
        })
      );
      const lastConversionRate = Number(
        result.getValue({
          name: 'formulapercent1',
          summary: search.Summary.MAX,
        })
      );
      const currentConversionRate = Number(
        result.getValue({
          name: 'formulapercent2',
          summary: search.Summary.MAX,
        })
...
const createSublist = (
  ...
  for (let i = 0; i < data.results.length; i++) {
    ...
    sublist.setSublistValue({
      id: 'custpage_field_' + widgetName + '_prev_num_of_leads',
      line: i,
      value: String(result.lastTotalLeads),
    });
    sublist.setSublistValue({
      id: 'custpage_field_' + widgetName + '_prev_converted_customers',
      line: i,
      value: String(result.lastConvertedLeads),
    });
    sublist.setSublistValue({
      id: 'custpage_field_' + widgetName + '_prev_conversion_rate',
      line: i,
      value: String(result.lastConversionRate) + '%',
    });
    sublist.setSublistValue({
      id: 'custpage_field_' + widgetName + '_prev_avg_days_to_close',
      line: i,
      value: String(result.lastAvgDaysToClose),
    });
    sublist.setSublistValue({
      id: 'custpage_field_' + widgetName + '_current_num_of_leads',
      line: i,
      value: String(result.currentTotalLeads),
    });
    sublist.setSublistValue({
      id: 'custpage_field_' + widgetName + '_current_converted_customers',
      line: i,
      value: String(result.currentConvertedLeads),
    });
    sublist.setSublistValue({
      id: 'custpage_field_' + widgetName + '_current_conversion_rate',
      line: i,
    ...
    sublist.setSublistValue({
      id: 'custpage_field_' + widgetName + '_current_avg_days_to_close',
      line: i,
      value: String(result.currentAvgDaysToClose),
    });
  }
