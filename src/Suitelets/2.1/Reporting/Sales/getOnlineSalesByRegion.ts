...
interface Row {
  region: string;
  amount: number;
  orderCount: FieldValue;
  avgOrderAmount: number;
  salesGrowth: string;
  lastSales: number;
  currentSales: number;
  lastOrderCount: FieldValue;
  currentOrderCount: number;
  lastAvgOrderAmount: number;
  currentAvgOrderAmount: number;
}
...
export const _get = (
  ...
  const westernData = getTransactionSearchResults(
    westernID,
    'Western',
    start,
    end,
  ...
  const centralData = getTransactionSearchResults(
    centralID,
    'Central',
    start,
    end,
  ...
  const easternData = getTransactionSearchResults(
    easternID,
    'Eastern',
    start,
    end,
  ...
  // combine
  const data = westernData.results
    .concat(centralData.results)
    .concat(easternData.results);
  // totals
  let totalAmount = 0;
  ...
  const totalsRow = {
    ...
    currentAvgOrderAmount: utils.getAvg(
      currentSalesTotal,
  ...
  return { results: formatTransactionResults(data) };

  // return createSublist(form, data, 'online_sales');
};

const getTransactionSearchResults = (
  searchID: string,
  region: string,
  start: string,
  end: string,
  ...
  pagedData.pageRanges.forEach(pageRange => {
    ...
    page.data.forEach(result => {
      ...
      const totalSales = Number(
        result.getValue({ name: 'amount', summary: search.Summary.SUM })
      );
      const totalAvgOrderAmount = Number(
        result.getValue({
          name: 'formulacurrency',
          summary: search.Summary.MAX,
        })
      );
      const lastSales = Number(
        result.getValue({
          name: 'formulacurrency1',
          summary: search.Summary.MAX,
        })
...
const formatTransactionResults = (results: Row[]) => {
  ...
  for (let i = 0; i < results.length - 1; i++) {
    formattedResults.push({
      ...
      avgOrderAmount: results[i].avgOrderAmount,
      ...
      lastAvgOrderAmount: utils.formatNumber(
        utils.round(results[i].lastAvgOrderAmount, 2)
      ),
      currentAvgOrderAmount: utils.formatNumber(
        utils.round(results[i].currentAvgOrderAmount, 2)
      ),
    });
  }
  // totals
  formattedResults.push({
    ...
    avgOrderAmount: '<b>' + results[results.length - 1].avgOrderAmount + '</b>',
    salesGrowth: '<b>' + results[results.length - 1].salesGrowth + '</b>',
    lastSales:
      '<b>' +
      utils.formatNumber(
        utils.round(results[results.length - 1].lastSales, 2)
      ) +
      '</b>',
    currentSales:
      '<b>' +
      utils.formatNumber(
        utils.round(results[results.length - 1].currentSales, 2)
      ) +
      '</b>',
    lastOrderCount: '<b>' + results[results.length - 1].lastOrderCount + '</b>',
    currentOrderCount:
      '<b>' + results[results.length - 1].currentOrderCount + '</b>',
    lastAvgOrderAmount:
      '<b>' +
      utils.formatNumber(
        utils.round(results[results.length - 1].lastAvgOrderAmount, 2)
      ) +
      '</b>',
    currentAvgOrderAmount:
      '<b>' +
      utils.formatNumber(
        utils.round(results[results.length - 1].currentAvgOrderAmount, 2)
      ) +
      '</b>',
  });
