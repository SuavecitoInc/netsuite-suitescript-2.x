...
interface Row {
  salesRep: string;
  lastTotalQuotes: string;
  lastConvertedQuotes: string;
  lastConversionRate: string;
  lastTotalAmount: string;
  currentTotalQuotes: string;
  currentConvertedQuotes: string;
...
const getTransactionSearchResults = (start, end, prevStart, prevEnd) => {
  ...
  const lastConvertedQuotes = search.createColumn({
    ...
    formula:
      ...
      "', 'MM/DD/YYYY') AND to_date('" +
      prevEnd +
      "', 'MM/DD/YYYY') AND {status} = 'Processed' THEN 1 ELSE 0 END),0)",
    summary: search.Summary.MAX,
  });
  ...
  const lastConversionRate = search.createColumn({
    ...
    formula:
      "CASE WHEN SUM(CASE WHEN {trandate} BETWEEN to_date('" +
      prevStart +
      "', 'MM/DD/YYYY') AND to_date('" +
      prevEnd +
      "', 'MM/DD/YYYY') THEN 1 ELSE 0 END) = 0 THEN 0 ELSE " +
      "NVL(SUM(CASE WHEN {trandate} BETWEEN to_date('" +
      prevStart +
      "', 'MM/DD/YYYY') AND to_date('" +
      prevEnd +
      "', 'MM/DD/YYYY') AND {status} = 'Processed' THEN 1 ELSE 0 END) / " +
      "SUM(CASE WHEN {trandate} BETWEEN to_date('" +
      prevStart +
      "', 'MM/DD/YYYY') AND to_date('" +
      prevEnd +
      "', 'MM/DD/YYYY') THEN 1 ELSE 0 END),0) END",
    summary: search.Summary.MAX,
  });
  ...
  const lastTotalTransAmount = search.createColumn({
    ...
    formula:
      ...
      "', 'MM/DD/YYYY') AND to_date('" +
      prevEnd +
      "', 'MM/DD/YYYY') AND {status} = 'Processed' THEN {amount} END),0)",
    summary: search.Summary.MAX,
  });
  ...
  pagedData.pageRanges.forEach(pageRange => {
    ...
    page.data.forEach(result => {
      ...
      const lastTotalQuotes = Number(
        result.getValue({
          name: 'formulanumeric1',
          summary: search.Summary.MAX,
        })
      );
      const currentTotalQuotes = Number(
        result.getValue({
          name: 'formulanumeric2',
          summary: search.Summary.MAX,
        })
      );
      const lastConvertedQuotes = Number(
        result.getValue({
          name: 'formulanumeric3',
          summary: search.Summary.MAX,
        })
      );
      const currentConvertedQuotes = Number(
        result.getValue({
          name: 'formulanumeric4',
          summary: search.Summary.MAX,
        })
      ...
      const lastTotalAmount = Number(
        result.getValue({
          name: 'formulanumeric5',
          summary: search.Summary.MAX,
        })
      );
      const currentTotalAmount = Number(
        result.getValue({
          name: 'formulanumeric6',
          summary: search.Summary.MAX,
        })
  ...
  const totalLastConversionRate =
    totalLastQuoteCount > 0
      ? utils.round((totalLastConvertedQuotes / totalLastQuoteCount) * 100, 2)
      : 0;

  const totalCurrentConversionRate =
    totalCurrentQuoteCount > 0
      ? utils.round(
          (totalCurrentConvertedQuotes / totalCurrentQuoteCount) * 100,
          2
        )
      : 0;
