...
const onGet = (response: ServerResponse) => {
  ...
  searchForm
    ...
    'Use a partial SKU below to find all matching items. This will then create an Inventory Adjustment to "zero" out all remaining quantities not in a "Picked" bin.';
...
const onPost = (request: ServerRequest, response: ServerResponse) => {
  if (request.parameters.custpage_partial_sku) {
    ...
    const invAdjId = inventoryAdjustment(
      request.parameters.custpage_set_partial_sku,
      items
    );
    // make inactive
...
const getItems = (partialSku: string) => {
  ...
  itemSearch.filters = [
    ...
    search.createFilter({
      ...
      values: [1],
      formula:
        'CASE WHEN NVL({locationquantityavailable},0) > 0 ' +
  ...
  results.forEach(function (item) {
    items.push({
      ...
      binOnHandAvailable: item.getValue({
        name: 'quantityavailable',
        join: 'binonhand',
      }),
...
const createPage = (partialSku: string, items: any[]) => {
  ...
  const form = serverWidget.createForm({
    title: 'Discontinue The Following SKU(s)',
  });
