...
export let onRequest: EntryPoints.Suitelet.onRequest = (
  ...
  const vt = runtime.getCurrentScript().getParameter({
    name: 'custscript_retail_store_create_po_vendor',
  });
 ...
 */
const onGet = (response: ServerResponse, vt: string) => {
  const items = getReplenishment();
  const page = createPage(vt, items);
  response.writePage(page);
};

const onPost = (response: ServerResponse, vt: string) => {
  const items = getReplenishment();
  ...
  form.addField({
    ...
    'Purchase Order created: <a href="https://system.netsuite.com/app/accounting/transactions/purchord.nl?id=' +
    purchaseOrderId +
...
const getReplenishment = () => {
  ...
  const retailReplenishmentSavedSearch = runtime
    .getCurrentScript()
    .getParameter({
      name: 'custscript_retail_store_create_po_search',
    });
  const retailStoreSearch = search.load({
    id: retailReplenishmentSavedSearch as string,
  });

  const qtyNeeded = search.createColumn({
    name: 'formulanumeric1',
    formula:
      'NVL({custrecord_rfs_replenishment_rule_item.custrecord_rfs_replenishment_rule_max},0)  - NVL({locationquantityavailable},0)',
  });
  ...
  pagedData.pageRanges.forEach(pageRange => {
    ...
    page.data.forEach(result => {
      ...
      itemResults.push({
        ...
        binName: result.getValue({
          name: 'custrecord_rfs_replenishment_rule_bin',
          join: 'custrecord_rfs_replenishment_rule_item',
        }),
        min: result.getValue({
          name: 'custrecord_rfs_replenishment_rule_min',
          join: 'custrecord_rfs_replenishment_rule_item',
        }),
        max: result.getValue({
          name: 'custrecord_rfs_replenishment_rule_max',
          join: 'custrecord_rfs_replenishment_rule_item',
        }),
 ...
 */
const createPage = (vt: string, items: any[]) => {
  const form = serverWidget.createForm({
    title:
      vt +
      ' - Retail Replenishment - ' +
  ...
  form.addField({
    ...
    'The following results do not take into account pending Purchase Orders. Please make sure your not creating duplicate POs.';
...
const createPurchaseOrder = (vt: string, items: any[]) => {
  ...
  purchaseOrder.setValue(
    'memo',
    'Auto Generated ' + vt + ' - Retail Store Purchase Order'
  );
